...
tactic mode
- We claimed it was a backward proof method, but this isn't exactly accurate. In DN -> EM, we did a lot of work
just to get back to the original goal with an extra fact in our context
- They were goal/conclusion directed
- Unstructured: Very difficult to understand without an editor in front of you

term mode
- Forward proof, context directed, structured
...
theorem fst_of_two_props :
  ...
  assume hb : b
  -- show a from ha
  have hpq : a ∧ b := And.intro ha hb
  show a from And.left hpq

/- Proof above but in tactic mode-/
theorem fst_of_two_props2 :
  ∀a b : Prop, a → b → a :=
  by
    intro P Q
    intro ha
...
theorem prop_comp2 (a b c : Prop) (hab : a → b) (hbc : b → c) :
  a → c := 
  by
    intro ha
    apply hbc
...
theorem prop_comp3 (a b c : Prop) (hab : a → b) (hbc : b → c) :
  a → c := 
  by
    intro ha
    have hb : b :=
...
theorem prop_comp_inline2 (a b c : Prop) (hab : a → b)
  (hbc : b → c) :
  a → c :=
  fun ha => hbc (hab ha)
...
add : {A : Type} -> verctor A n → vector A n → vector A n

Fin n = {x : ℕ // x ≤ n} (here the type Fin n depends on the natural numbers)
